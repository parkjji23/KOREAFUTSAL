...
public class BoardDAO {
	...
	public ArrayList<GalBoardDto> selectArticleList(int page, int limit) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String board_list_sql = "select * from galboard_tb order by G_B_NO desc limit ?,12";
	...
	public ArrayList<GalBoardDto> selectArticleListTeam(int page, String t_code) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String board_list_sql = "select * from galboard_tb where T_CODE = ? order by G_B_NO desc limit ?,12";
		ArrayList<GalBoardDto> articleList = new ArrayList<GalBoardDto>();
		GalBoardDto board = null;
	...
	public ArrayList<GalRepDto> selectArticleRepList(int num) {
		...
		String board_list_sql = "select * from galrep_tb where G_B_NO =? order by G_REP_DATE desc, G_REP_NO desc";
	...
	public ArrayList<NoticeboardDto> selectnoticelist(){
		
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String board_list_sql="SELECT * FROM noticeboard_tb where N_STATE= '팀갤러리 공지사항'";
		...
		try{
			...
			while(rs.next()){
				board = new NoticeboardDto();
				board.setAdmin_name(rs.getString("admin_name"));
				System.out.println("확인 어드민아이디는? "+board.getAdmin_name());
				...
				board.setN_title(rs.getString("N_STATE"));
				board.setN_num(rs.getInt("N_NUM"));
				noticelist.add(board);
				System.out.println("값은?"+board);
