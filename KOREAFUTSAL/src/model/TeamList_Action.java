...
public class TeamList_Action {
	...
	public List<TeamDto> getTeamList(TeamPageDto teamPageDto){
		...
		System.out.println("selectTeamList의 팀페이지"+teamPageDto.getPage());
		TeamList = sqlSession.selectList("getteamlist",teamPageDto);
		sqlSession.close();
		return TeamList;
	}
	...
	public int getListCount(TeamPageDto teamPageDto) {
		int listCount = 0;
		SqlSession sqlSession = factory.openSession();
		listCount=sqlSession.selectOne("teamtotalcount",teamPageDto);
		sqlSession.close();
		return listCount;
	}
