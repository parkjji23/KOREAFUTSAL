...
public class TeamSearchAction {
	...
	public List<TeamDto> searchinform(TeamDto teamlist) {
		System.out.println("TeamSearchAction 접근");
		...
		if(obb.getTeamtype().equals("전체")||obb.getBranchcode().equals("전국현황")) {
			list = sqlSession.selectList("searchteamlistall", obb);       // sqlSession 을 불러온 DB를 쓸건데 거기서 selectList 쓴다. mapper 파일에서 태그 이름 ! 
		}
		else {
