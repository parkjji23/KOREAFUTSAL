			...
			<div id="matList">
				...
				<div class="rese_start">
					...
					<div class="start_wrap">
						<div class="cont">
							<form id="reg_form" name="reg_form" method="post" action="league.do?command=league_team_step2">
							...
							<div class="btn_wrap">
								<a href="javascript:void(0);" onClick="ajaxteamlist()" class="btn_big">대회 접수</a>
							</div>
						</div>
					</div>
					<script>
						function ajaxteamlist(){
							$.ajax({
								type : "post",
								url : "./leaderCheck.do?command=leaderCheck",
								data : {
									id : $('#id').val()
								},
								success : whenSuccessleader,
								error : whenErrorleader
							});
						}
						function whenSuccessleader(resdata) {
							var id = '<%=uss%>'
							console.log(id);
							if(id!="GUEST"){
								if (!resdata.includes("leader")) {
									alert("팀장으로 등록된 회원이 아닙니다.");
								}else {
									leaguesubmit();
								}
							}else{
