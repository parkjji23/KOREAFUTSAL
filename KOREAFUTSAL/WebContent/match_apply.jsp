...
<%
	...
	ArrayList<MatchlistDto> matchlist = (ArrayList<MatchlistDto>) request.getAttribute("matchlist");
...
<!----------------------------오버레이 화면 띄우는 스크립트------------------------------------>
<script>						
	function selectOne(index,mid) {
		$.ajax({
		type:"get",
		
		url:"./matchSelectOne.jsp",       
		data:{
		...
		<section id="mainright">
			<div id="matList">
				<h3 class="h_tit" style="font-weight:normal;">매 치 신 청 </h3>
					<input class="signbutton signbutton6 " type="button" value="매치등록"
								style="float: right; margin-right: 33px;"
								onClick="location.href='matyong.do?command=match_apply_reg'">
				<div class="cbox">
				...
				<!-- -----------------리스트------------------------ -->
						...
						<div class="teams_table" style="clear: both; width: 950px;">
							<div class="search">
								<form name="frm_search" method="post"
									...
									action="matyong.do?command=matchapply">
							...
							<div class="table_wrap">
								<table>
									...
									<tbody>
										...
										%>
										<tr>
											<td class="tb"><%=matchlist.get(i).getM_b_branch()%></td>
											<td>
												<div class="emb_wrap">
													<a onClick="selectOne(<%=matchlist.get(i).getM_b_no()%>,'<%=matchlist.get(i).getId()%>'); "
														class="button" id="listtt" style="color: #337ab7;"> <%=matchlist.get(i).getM_b_schedule()%></a>
												</div>
											</td>
											<td><%=matchlist.get(i).getM_b_time()%></td>
											<td class="tb"><%=matchlist.get(i).getM_b_name()%></td>
											<td><%=matchlist.get(i).getM_b_status()%></td>
										</tr>
										<%
											}
											}
					...
					<ul class="pagination">
						...
						<li><a href="matyong.do?command=matchapply&page=<%=nowPage - 1%>">이전</a></li>&nbsp;
						<%
							}
						%>
						...
						%>
						<li><a href="matyong.do?command=matchapply&page=<%=a%>"><%=a%>
						</a></li>&nbsp;
						<%
							}
						%>
						...
						%>
						<li><a href="matyong.do?command=matchapply&page=<%=nowPage + 1%>">다음</a></li>
						<%
							}
						%>
						</ul>
