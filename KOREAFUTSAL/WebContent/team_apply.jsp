...
<!-- 가입 신청자 명단 리스트 -->
<% 
	ArrayList<PlayerRegistDto> Applyer = (ArrayList<PlayerRegistDto>) session.getAttribute("Applyer");
	ArrayList<MemberDto> Member = (ArrayList<MemberDto>) session.getAttribute("Member");
	%>
<!doctype html>
<html lang="en">
<head>
...
function team_join_approval(aid) {
		var aid= aid;
		alert(aid);		
			$.ajax({
				type : "post",
				url : "./teamjoin.do?command=teamjoin",
				dataType : "json",
				data : {	
					team_code : team_code,
					id : aid,
				...
				success : whenSuccessjoinapproval,
				error : whenErrorjoinapproval
			});
			
	}
function whenSuccessjoinapproval(resdata) {
	
	var leagueteamresult = resdata[0].leagueteamresult;
	
	
		alert("가입 신청을 수락했습니다.");
		location.href="team_info.do?command=teaminfo&teamcode="+team_code;

}
function whenErrorjoinapproval() {
	
	
	alert(team_name + "출력 내용이 true false 둘다 아닙니다.");
}


function team_join_refusal(aid) {
		var aid= aid;
		alert(aid);		
			$.ajax({
				type : "post",
				...
				success : whenSuccessjoinrefusal,
				error : whenErrorjoinrefusal
			});
			
	}
function whenSuccessjoinrefusal(resdata) {
				...
				<div class="cbox">
					...
					<div class="tab_wrap">
						<ul class="t04">
							...
							<li  class="on"><a href="#"> 가입신청명단</a></li>
						</ul>
					</div>
					...
					<div class="league_schedule">
						...
						<div class="table_wrap">
							...
							<colgroup>
								...
								<col style="width:80px" class="w01">
								<col style="width:80px" class="w01">
								<col style="width:120px" class="w06">
								<col style="width:80px" class="w01">
							</colgroup>
							...
							<%for(int i=0; i<Member.size();i++){ %>
								<tr>
									...
									<td>
										<%=Member.get(i).getPosition() %>
									</td>
									<td>
										<%=Member.get(i).getTel() %>
									</td>
									<td>
									<input class="signbutton signbutton5" type="button" value="승인" style="float: left; width : 110px; margin: 10px; font-size :15px;" onClick=team_join_approval('<%=Applyer.get(i).getId() %>');>
									<input class="signbutton signbutton5" type="button" value="거절" style="float: left; width : 110px; margin: 10px; font-size :15px;" onClick=team_join_refusal('<%=Applyer.get(i).getId() %>');>
									</td>
