...
<%
	...
	ArrayList<GalRepDto> articlelist = (ArrayList<GalRepDto>) request.getAttribute("articlelist");
		...
		<section id="mainright">
			<div id="matList">
				...
				<div class="container" id="formOutterWrapper">
						...
						<h3 class="h_tit" style="font-weight:normal;">갤러리 상세보기</h3>
						

						<div class="container" id="formInnerWrapper">

							<table>
								<tr style="padding: 15px 15px 22px 15px; ;padding-left: 40px;border-top: 2px solid #2c3c57;">
									
									<td colspan="2" class="viewform" style="font-size: 15px;">
										<div class="viewsubject" style="float:left;padding-top: 10px; padding-bottom: 5px; width:100%;"><%=article.getG_b_subject()%></div>
										<div style="text-align: right;"><%=article.getG_b_date() %></div></td>
								</tr>
								<tr style="padding: 15px 15px 22px 15px;border-bottom: 1px solid #d5d8dd;border-top: 1px solid #d5d8dd;">
									
									<td class="viewform" colspan="2" style="font-size:20px;padding-left: 10px;padding-top: 5px;">작성자 : <%=article.getId()%></td>
								</tr>
								<tr>
									<td rowspan="2" class="viewsubject"  style="text-align: center;border-bottom: 2px solid #2c3c57;">내용</td>
									<td class="viewform" style="padding-top:50px;">
										<%
											if (!(article.getG_b_file() == null)) {
										%>
										<a href="gal_file_down.jsp?downFile=<%=article.getG_b_file()%>"> 
										<img alt="" src="img/<%=article.getG_b_file()%>"  class="imgst">
										</a>
										<%
											}
										%>
								...
								<tr style="border-bottom: 2px solid #2c3c57;">
									...
									<td class="viewform" style="font-size: 25px;height: 100px;">
										<%=article.getG_b_contents()%>
									</td>
								</tr>
							</table>
							...
							<div id="rep">
								...
								<input type="text" name="GB_REP_CONTENTS" id="GB_REP_CONTENTS" />
								<input type="submit" id="repSubmit" class="signbutton signbutton5" value="답글" style="margin-left: 6px;"/>
								<input type="hidden" name="GB_NUM" value="<%=article.getG_b_no() %>" style="position:fixed;" />
								<input type="hidden" name="GB_ID" value="<%=article.getId() %>" style="position:fixed;" />
								<input type="hidden" name="page" value="<%=nowPage %>" style="position:fixed;" />
							...
							<div id="innerRep">
								...
								%>	
										<tr>
											...
											<td id="repContents"><%=articlelist.get(i).getG_rep_contents() %></td>
											<td id="repId"><%=articlelist.get(i).getId() %></td>
											<td id="repDate"><%=articlelist.get(i).getG_rep_date() %></td>
										</tr>
								<% 
									}
								%>
